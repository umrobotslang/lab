...
function factory.createLevelApi(kwargs)
  ...
  local possibleGoalLocations, otherGoalLocations = parsePossibleGoalLocations(maze)
  local api = {}
  ...
  function api:start(episode, seed, params)
    ...
    api._last_pose = tensor.DoubleTensor{spawn_x, spawn_y, 0, 0, 0, 0}
  ...
  function api:nextMap()
    local x, y = api._last_pose(1):val(), api._last_pose(2):val()
    local spawn_key = x .. ' ' .. y
    ...
    api._newSpawnVarsPlayerStart = {
       ...
       , origin = spawn_key .. ' 30'
       , angle = '' .. rad2deg(api._last_pose(5):val())
       , randomAngleRange = '0'
    }
  ...
  function api:customObservation(name)
    local co = customObservation(api, name)
    if name == 'POSE' then
       api._last_pose = co
    end
    return api._obs_value[name] or co
  end
