...
function api:start(episode, seed)
  ...
  api.mapNames = {"rw_i_maze_goal", "rw_i_maze_fungi"}
...
function api:createPickup(className)
  ...
  if (api.mapName == "rw_i_maze_fungi") then
...
function api:nextMap()
    local chosen_map = random.uniformInt(1,2)
    api.mapName = api.mapNames[chosen_map]
    return api.mapName
end
...
api._obs_value = {}
if (api.mapName == "rw_i_maze_goal") then
